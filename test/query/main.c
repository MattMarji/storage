...
START_TEST (test_mixed_col_order)
{
	//Do a query. Input predicate with columns not in order. Should return 2.
	int foundkeys = storage_query(SIXCOLSTABLE, "col2 = ABC , col1 = abc", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
...
{
	//Do a query. Input float number for int type. Expect an error.
	int foundkeys = storage_query(FOURCOLSTABLE, "col2 > 1.0", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
	fail_unless(foundkeys == -1, "storage_query with float should fail.");
	
	//Errno should be set correctly.
	fail_unless(errno == ERR_INVALID_PARAM, "storage_query with float not setting errno properly.");
}
END_TEST


START_TEST (test_operator_for_string_type)
{
	//Do a query. Input incorrect operator for string type. Expect an error.
	int foundkeys = storage_query(THREECOLSTABLE, "col3 > abc", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
	fail_unless(foundkeys == -1, "storage_query with incorrect operator should fail.");

	//Errno should be set correctly.
	fail_unless(errno == ERR_INVALID_PARAM, "storage_query with incorrect operator for string type not setting errno properly.");
}
END_TEST


START_TEST (test_col_name)
{
	//Do a query. Input column name 'col1' as 'Col1'. Expect an error.
	int foundkeys = storage_query(THREECOLSTABLE, "Col1 > 1", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
	fail_unless(foundkeys == -1, "storage_query with Col column name should fail.");

	//Errno should be set correctly.
	fail_unless(errno == ERR_INVALID_PARAM, "storage_query with incorrect column not setting errno properly.");
}
END_TEST
...
int main(int argc, char *argv[])
{
	...
	Suite *s = suite_create("Query Tests");
	TCase *tc;
	...
	tcase_add_test(tc, test_mixed_col_order);
	tcase_add_test(tc, test_float_type);
	tcase_add_test(tc, test_operator_for_string_type);
	tcase_add_test(tc, test_col_name);
