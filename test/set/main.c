...
START_TEST (test_col_order)
{
	//Do set. Input columns in incorrect order. 
	struct storage_record record;
	strncpy(record.value, "col2 1 , col1 1 , col3 merp", sizeof record.value);
	int status = storage_set(THREECOLSTABLE, KEY1, &record, test_conn);
	fail_unless(status == -1, "storage_set with incorrect column order should fail.");
	
	//Check if errno is set correctly.	
	fail_unless(errno == ERR_INVALID_PARAM, "storage_set with incorrect column order not setting errno properly.");
}
END_TEST


START_TEST (test_value_type)
{
	//Do set. Input incorrect value types. 
	struct storage_record record;
	strncpy(record.value, "col1 loser , col2 1 , col3 merp", sizeof record.value);
	int status = storage_set(THREECOLSTABLE, KEY1, &record, test_conn);
	fail_unless(status == -1, "storage_set with incorrect value types should fail.");
...
START_TEST (test_extra_col)
{
	//Do set. Input extra column. 
	struct storage_record record;
	strncpy(record.value, "col1 -2 , col2 -2 , col3 abc , col4 5", sizeof record.value);
	int status = storage_set(THREECOLSTABLE, KEY1, &record, test_conn);
...
{
	//Do set. Input incorrect column name (case sensitive). 
	struct storage_record record;
	strncpy(record.value, "Col1 -2 , col2 -2 , col3 abc", sizeof record.value);
	int status = storage_set(THREECOLSTABLE, KEY1, &record, test_conn);
	fail_unless(status == -1, "storage_set with incorrect column name should fail.");

	//Check if errno is set correctly.	
	fail_unless(errno == ERR_INVALID_PARAM, "storage_set with incorrect column name not setting errno properly.");
}
END_TEST


START_TEST (test_float_type)
{
	//Do set. Input float type. Expect an error.
	struct storage_record record;
	strncpy(record.value, "col1 2.0 , col2 -2 , col3 abc", sizeof record.value);
	int status = storage_set(THREECOLSTABLE, KEY1, &record, test_conn);
	fail_unless(status == -1, "storage_set with float type should fail.");

	//Check if errno is set correctly.	
	fail_unless(errno == ERR_INVALID_PARAM, "storage_set with float type not setting errno properly.");
}
END_TEST


START_TEST (test_missing_comma)
{
	//Do set. Input value without comma. Expect an error.
	struct storage_record record;
	strncpy(record.value, "col1 2 col2 -2 , col3 abc", sizeof record.value);
	int status = storage_set(THREECOLSTABLE, KEY1, &record, test_conn);
	fail_unless(status == -1, "storage_set without comma should fail.");

	//Check if errno is set correctly.	
	fail_unless(errno == ERR_INVALID_PARAM, "storage_set without comma not setting errno properly.");
}
END_TEST


START_TEST (test_add_modify_delete)
{
	//Do set. Input new record to the table. 
	struct storage_record record;
	strncpy(record.value, "col1 -7 , col2 2 , col3 abc", sizeof record.value);
	int status = storage_set(THREECOLSTABLE, KEY, &record, test_conn);
	fail_unless(status == 0, "Error inputting a record.");

	//Modify an existing record. 
	strncpy(record.value, "col1 -7 , col2 2 , col3 def", sizeof record.value);
	status = storage_set(THREECOLSTABLE, KEY, &record, test_conn);
	fail_unless(status == 0, "Error modifying record.\n");
	fail_unless(strcmp(record.value, "col1 -7 , col2 2 , col3 def") == 0, "Record was not modified.");

	//Delete a record.
	status = storage_set(THREECOLSTABLE, KEY, NULL, test_conn);
	fail_unless(status == 0, "Error deleting the record.");
}
END_TEST
...
int main(int argc, char *argv[])
{
	...
	tcase_add_test(tc, test_col_order);
	tcase_add_test(tc, test_value_type);
	tcase_add_test(tc, test_extra_col);
	tcase_add_test(tc, test_invalid_colname);
	tcase_add_test(tc, test_float_type);
	tcase_add_test(tc, test_missing_comma);
	tcase_add_test(tc, test_add_modify_delete);
